...
import type { Extension, ExtensionFactory, ExtensionRuntime, LoadExtensionsResult } from "./extensions/index.ts";
import { createExtensionRuntime, loadExtensionFromFactory, loadExtensions } from "./extensions/index.ts";
...
export class DefaultResourceLoader implements ResourceLoader {
	...
	private getDefaultSourceInfoForPath(filePath: string): SourceInfo {
		...
		const projectRoots = [
			join(getProjectDir(this.cwd), "skills"),
			join(getProjectDir(this.cwd), "prompts"),
			join(getProjectDir(this.cwd), "themes"),
			join(getProjectDir(this.cwd), "extensions"),
		];
	...
	private loadThemes(
		...
		if (includeDefaults) {
			const defaultDirs = [join(this.agentDir, "themes"), join(getProjectDir(this.cwd), "themes")];
