...
import type { ExtensionFactory } from "./core/extensions/index.ts";
...
async function promptForProjectTrust(
	...
	return showStartupSelector(
		settingsManager,
		`Trust project folder?\n${cwd}\n\nThis allows tau to read project instructions (AGENTS.md/CLAUDE.md), load project settings and resources, install missing project packages, and execute project extensions.`,
		[
...
function loadTauEnv(): void {
	try {
		const envPath = join(homedir(), ".tau", ".env");
...
export async function main(args: string[], options?: MainOptions) {
	...
	if (parsed.export) {
		console.error(chalk.red("Error: --export is not supported. Session export has been removed."));
		process.exit(1);
	}
	...
	if (appMode !== "interactive" && !session.model) {
		console.error(chalk.red("No models available. Set OPENROUTER_API_KEY environment variable."));
		process.exit(1);
	}
