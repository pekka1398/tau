...
describe("DefaultPackageManager", () => {
	...
	beforeEach(() => {
		previousOfflineEnv = process.env.TAU_OFFLINE;
	...
	afterEach(() => {
		if (previousOfflineEnv === undefined) {
			...
			process.env.TAU_OFFLINE = previousOfflineEnv;
		}
		vi.restoreAllMocks();
		vi.unstubAllGlobals();
...
Content`,
		...
		it("should auto-discover root markdown skills from .tau skill dirs", async () => {
...
Content`,
	...
	describe("auto-discovered skill metadata", () => {
		it("should use the agent dir as baseDir for user .tau/agent skills", async () => {
	...
	describe(".agents/skills auto-discovery", () => {
		...
		it("should dedupe user skill entries when ~/.tau/agent/skills is a symlink to ~/.agents/skills", async () => {
			const previousHome = process.env.HOME;
			process.env.HOME = tempDir;
	...
	describe("command spawning", () => {
		it("should preserve argv entries containing spaces", () => {
			...
			const valueWithSpace = "C:\\Users\\A B\\.tau\\npm";
	...
	describe("settings source normalization", () => {
		...
		it("should store project local packages relative to .tau settings base", () => {
